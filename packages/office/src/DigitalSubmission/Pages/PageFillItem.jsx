...
import { UpdateAsyncAction as UpdateSubmissionFieldAsyncAction } from "../../DigitalSubmissionFieldGQLModel/Queries/UpdateAsyncAction";
import { ReadAsyncAction as ReadSubmissionAsyncAction } from "../Queries";
import { CreateDelayer } from "@hrbolek/uoisfrontend-shared";
import { Link } from "../../../../_template/src/UserGQLModel/Components";
...
const FormSectionSections = ({
    ...
    const handleAddSubmissionSection = useCallback(
        async (formSectionDef) => {
            const payload = {
                ...
                submissionId: digital_submission_section.submission.id,
                sections: [],
                fields: [],
            };
            const result = await ctxInsert.run(payload)
            console.log("FormSectionSections.handleAddSubmissionSection", result)
    ...
    return (
        <>
            <AsyncStateIndicator error={ctxInsert.error} loading={ctxInsert.loading} text={"Vkládám"} />
            <AsyncStateIndicator error={ctxDelete.error} loading={ctxDelete.loading} text={"Odstraňuji"} />
...
const FormSectionBody = ({
    ...
    return (<>
        {/* {JSON.stringify(digital_submission_section?.fields)} */}
...
const UpdateSectionWrap = ({
    ...
    if (digital_submission_sections.length === 0) {
        return (<>
            {digital_submission_sections.length}
            <FormSectionComponent {...props} digital_submission_section={{
    ...
    return (<>
        {/* SS{JSON.stringify(digital_submission_sections)} */}
            ...
            {debug && (
                <Col>
                    <SimpleCardCapsule title="Draft form definition">
                        <pre>{JSON.stringify(initialSubmission?.value, null, 2)}</pre>
                    </SimpleCardCapsule>
                    <SimpleCardCapsule title="Draft submission">
                        <pre>{JSON.stringify(initialSubmission, null, 2)}</pre>
                    </SimpleCardCapsule>
                    {/* <SimpleCardCapsule title="Draft submission">
